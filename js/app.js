(() => {
    ...
    function rangeInit() {
        ...
        if (rangeItems.length) rangeItems.forEach((rangeItem => {
            ...
            const sliderType = fromValue.getAttribute("data-slider-type");
            const formatTooltip = {
                to: value => `${Math.floor(value)} ${sliderType}`,
                from: value => value
            };
            const slider = initialize(item, {
            ...
            const tooltips = item.querySelectorAll(".noUi-tooltip");
            ...
            slider.on("update", ((values, handle) => {
                ...
                inputs[handle].value = formatTooltip.to(values[handle]);
            }));
        }));
    }
    rangeInit();
